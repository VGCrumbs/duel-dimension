...
public class ChooseProgramScreen extends Screen
{
    ...
    {
        if(button == 0 && openList != null && grabScrollbar(mouseX, mouseY))
        {
            return true;
        }
        ...
        int mouseY, float partialTick)
    {
        ...
        // the human half of the interface. Pinned rather than drawn from its own
        // textures, so the Duel Bot gets the whole theme -- panel, buttons,
        // scrollbar and every colour of type -- from one line.
        //
        // In a finally, and around the super call too: the widgets paint in
        // there, and an exception part way through a frame must not leave every
        // other menu in the game wearing blue.
     ...
     * {@code super.render} and vanilla draws the background from inside it. In a
     * level that background is the BLUR and nothing else, so leaving it in place
     * blurs everything this screen has already put down.
     */
    @Override
