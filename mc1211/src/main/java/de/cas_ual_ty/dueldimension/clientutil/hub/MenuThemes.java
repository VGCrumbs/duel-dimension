...
public final class MenuThemes
{
    ...
    private static final Map<String, ResourceLocation> BUILT = new HashMap<>();
    ...
    private static void forget()
    {
        ...
        List<ResourceLocation> going = new ArrayList<>(BUILT.values());
        BUILT.clear();
        for(ResourceLocation id : going)
        {
     ...
     * not one.
     */
    public static ResourceLocation resolve(ResourceLocation texture)
    {
        ...
        {
            ResourceLocation built = BUILT.get(key);
            return built != null ? built : texture;
        }
        ResourceLocation built = build(theme, texture, key);
        BUILT.put(key, built);
        return built != null ? built : texture;
    }

    private static ResourceLocation build(MenuTheme theme, ResourceLocation master, String key)
    {
        Minecraft client = Minecraft.getInstance();
        try(InputStream stream = client.getResourceManager().open(master))
        {
            ...
            for(int y = 0; y < source.getHeight(); y++)
            {
                ...
                {
                    out.setPixelRGBA(x, y, abgr(paint(theme, abgr(
            ...
            client.getTextureManager().register(id, new DynamicTexture(out));
            return id;
        }
     ...
     * <p>
     * 1.21.1's {@code getPixelRGBA} is a misnomer -- the int is packed ABGR, in
     * memory order. Everything above works in ARGB, so the two ends convert.
     */
