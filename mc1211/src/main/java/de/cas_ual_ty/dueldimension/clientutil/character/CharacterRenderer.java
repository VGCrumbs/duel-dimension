 ...
 * they last asked for: the walk speed that swings the vanilla model's arms is
 * the same number that picks between standing, walking and running. Nothing new
 * has to be synced for a character to walk when its player walks.
 * <p>
 * <b>The player, not a render state.</b> This is where the port differs from
 * 26.2 and it differs by being simpler: 26.2 extracts a state per frame and the
 * renderer never sees the entity again, so the look has to be carried across on
 * the state itself ({@code CharacterCarrier}). 1.21.1 hands the renderer the
 * player, so it is looked up here and there is no carrier and no state mixin.
 */
public final class CharacterRenderer
{
    ...
    /**
     * How far a duellist has to have moved in a tick to be walking, squared.
     * <p>
     * A thousandth of a block, which is the same threshold {@link #backwards}
     * uses and for the same reason: below it nobody is going anywhere, and a
     * walk cycle that started on less would start every time somebody turned on
     * the spot.
     * <p>
     * Only the standing case needs a number. Walking and running are told apart
     * by the sprint flag, which Minecraft keeps deliberately -- a threshold
     * could not, since an ordinary walk reads 0.86 of the eased speed and a
     * sprint 1.00, and any line between those two is a hair's breadth wide.
     */
     ...
     */
    private static boolean crouching(net.minecraft.client.player.AbstractClientPlayer player)
    {
    ...
    private static void hold(PoseStack pose, ModelSkeleton skeleton, float[] bones,
        net.minecraft.world.item.ItemStack stack, boolean right,
        SubmitNodeCollector collector, int light, ItemAnchor.Kind shown)
    {
        ...
        grip(pose, skeleton, bones, right ? HAND_RIGHT : HAND_LEFT, !right,
            shown != null ? shown : ItemAnchor.kindOf(stack));
        net.minecraft.client.Minecraft.getInstance().getItemRenderer().renderStatic(
            stack,
            // RIGHT for both hands, deliberately. Vanilla's left-hand context
            // is the right-hand one with the item's own display rotation
            // negated -- a mirror of its own -- and this code already mirrors
            // the whole frame. Two mirrors cancel, which is why the left hand
            // came out not quite a reflection of the right. One mirror, applied
            // where the geometry is, and the item's transform is the same in
            // both hands.
     ...
     *
     * @param player      the player being drawn, which is where the walk speed
     *                    comes from
     * @param bodyYaw     which way their body faces, in degrees, as the
     *                    renderer was handed it
     * @param partialTick where between two ticks this frame is, so the walk
     *                    speed does not step
     * @param packedLight the light the vanilla body would have been drawn at
     */
    public static void submit(AbstractClientPlayer player, PoseStack poseStack,
        SubmitNodeCollector collector, CharacterLook look, float bodyYaw,
        float partialTick, int packedLight)
    {
        ...
        // the render this replaces. Cancelling at its head therefore cancels
        // the turn too, so the yaw has to be handed on here or every character
        // in the world faces south while walking north.
        ResourceLocation[] painted = new ResourceLocation[SLOTS.length];
        ...
        String moving = clipFor(player, look, partialTick);
        RideAnchor.Seat seat = seat(player.isPassenger());
        // Swinging: the strike, from the instant it started. Not swinging: NaN,
        // which is what tells the renderer to loop rather than to play once.
        boolean hitting = player.swinging;
        String clip = hitting ? (look.female() ? "fpa" : "mpa") + HIT : moving;
        float seconds = hitting ? (player.swingTime + partialTick) / 20F : Float.NaN;
        ...
        // -- the dispatcher put it there -- so the translate inside cancels to
        ...
        de.cas_ual_ty.dueldimension.clientutil.model.ModelLight light =
            de.cas_ual_ty.dueldimension.clientutil.model.ModelLight.flat(packedLight);

        // The body's rotation, not the head's: a player's head turns to look
        // about while their body keeps walking, and it is the body the model
        // stands on. Interpolated, because 1.21.1 hands it to the renderer
        // already interpolated and that is the number the vanilla model uses.
        ModelHologram.submit(poseStack, collector, eye, eye, HEIGHT, mesh,
            bodyYaw, 0xFFFFFFFF, clip, seat.y(), seat.yaw(), seat.x(), seat.z(),
            seconds, light,
            part ->
            {
            ...
            moving, turns(player, partialTick, mesh, seat), HIT_ARM,
            backwards(player, bodyYaw),
            ...
            (pose, skeleton, bones) ->
            {
                boolean tuning = net.minecraft.client.Minecraft.getInstance().screen
                ...
                ItemAnchor.Kind shown = tuning
                    ...
                    : null;
                hold(pose, skeleton, bones, player.getMainHandItem(),
                    player.getMainArm() == net.minecraft.world.entity.HumanoidArm.RIGHT,
                    collector, packedLight, shown);
                hold(pose, skeleton, bones, player.getOffhandItem(),
                    player.getMainArm() != net.minecraft.world.entity.HumanoidArm.RIGHT,
                    collector, packedLight, shown);
                // The handles, only while their editor is open and only on the
                // player editing them. An empty hand is exactly when the anchor
                // is hardest to see, so these are not gated on holding anything.
                if(tuning)
                {
            ...
            ItemAnchor.kindOf(player.getMainHandItem()) == ItemAnchor.Kind.TOOL
     ...
     */
    private static ModelSkeleton.Turn[] turns(AbstractClientPlayer player, float partialTick,
        ModelMesh mesh, RideAnchor.Seat seat)
    {
        // A rider does not crouch: the seated clip already has the legs, and
        // folding them under a horse would take them through it.
        boolean crouching = crouching(player);
        ModelSkeleton.Turn[] head = look(player, partialTick);
     ...
     *
     */
    private static ModelSkeleton.Turn[] look(AbstractClientPlayer player,
        float partialTick)
    {
        // Interpolated on both sides, because 1.21.1 stores last tick and
        // this tick rather than handing the renderer a settled number:
        // a head read at whole ticks steps where the body glides.
        ...
        float yaw = Math.clamp(Mth.wrapDegrees(head - body), -YAW_LIMIT, YAW_LIMIT);
        // Minecraft's pitch is positive DOWNWARD, so the ventral limit is
        // the positive end and the dorsal one the negative.
        float pitch = Math.clamp(Mth.lerp(partialTick, player.xRotO, player.getXRot()),
            -LOOK_UP, LOOK_DOWN);
    ...
    /**
     * Is this duellist travelling backwards relative to the way they face?
     * <p>
     * The dot of where they MOVED against where they are POINTED. Minecraft
     * keeps the body facing forwards while strafing or backing up, so the same
     * walk cycle has to serve every direction, and running it forwards while its
     * owner goes the other way is the moonwalk.
     * <p>
     * Read off the change in position rather than off any input, because this
     * has to be true of every player on the server and only one of them is
     * holding the keys. Below a threshold it answers false -- a duellist turning
     * on the spot moves a few thousandths either way, and a walk cycle that
     * flipped direction on that would stutter rather than turn.
     */
    /**
     ...
     */
    private static float decay(AbstractClientPlayer player)
    {
     ...
     * the ice did.
     */
    private static float holding(AbstractClientPlayer player)
    {
     ...
     */
    private static boolean striding(AbstractClientPlayer player)
    {
     ...
     */
    private static boolean carried(AbstractClientPlayer player)
    {
        return !striding(player);
    }

    private static boolean backwards(AbstractClientPlayer player, float bodyYaw)
    {
        // Only a walk cycle has a direction to get wrong. Reversing the seated
        // clip because the horse backed up would run the rider's idle sway
        // backwards, which is not a thing anybody would notice going right and
        // is very odd going wrong.
        ...
        // Minecraft's yaw counts from SOUTH, so forward is (-sin, cos).
        double radians = Math.toRadians(bodyYaw);
        return dx * -Math.sin(radians) + dz * Math.cos(radians) < 0D;
    }
     ...
     */
    /**
     * The clip this duellist is playing, for anything outside the renderer that
     * has to agree with it.
     * <p>
     * {@link FootSteps} needs it: a footstep lands where the animation says it
     * lands, so the sound has to be timed against the same clip the picture is
     * posed from, chosen by the same rule.
     */
    public static String clipOf(AbstractClientPlayer player, CharacterLook look)
    {
        return clipFor(player, look, 1F);
    }

    /** The rate that clip is running at. See {@link #rateFor}. */
    public static float rateOf(AbstractClientPlayer player, String clip)
    {
        return rateFor(clip, crouching(player));
    }
    ...
    private static String clipFor(AbstractClientPlayer player, CharacterLook look,
        float partialTick)
    {
        ...
        //
        // `walkAnimation.speed` is eased -- update(f, 0.4F) -- so it decays
        // towards zero over several ticks after the keys are let go, and a
        // threshold on it keeps the walk running for most of a second after a
        // duellist has stopped. The position told the truth the whole time.
        double dx = player.getX() - player.xOld;
        double dz = player.getZ() - player.zOld;
        if(dx * dx + dz * dz < MOVED || carried(player))
        {
            return prefix + IDLE;
        }
