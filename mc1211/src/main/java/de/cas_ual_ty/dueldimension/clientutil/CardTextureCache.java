 ...
 * ResourceLocation goes back to {@link CardImageManager.preloadStatus#NONE} in the
...
public final class CardTextureCache
{
    ...
    static final Map<ResourceLocation, Integer>[] RESIDENT = new Map[CardImageManager.CLASSES];
     ...
     */
    private static final Map<ResourceLocation, Long>[] TOUCHED_AT = newTouchMaps();

    @SuppressWarnings("unchecked")
    private static Map<ResourceLocation, Long>[] newTouchMaps()
    {
        Map<ResourceLocation, Long>[] maps = new Map[CardImageManager.CLASSES];
     ...
     * ResourceLocation counted against the budget for ever while occupying no VRAM,
     ...
     */
    public static void touch(ResourceLocation id, int size)
    {
        if(id == null)
        {
            return;
    ...
    interface Releaser
    {
        void release(ResourceLocation id, int sizeIndex);
    }
     ...
     * an ResourceLocation that {@code TextureManager} no longer holds, and the next
    ...
    static void sweep(Releaser releaser)
    {
        for(int index = 0; index < CardImageManager.CLASSES; index++)
        {
            ...
            synchronized(resident)
            {
                ...
                for(Iterator<Map.Entry<ResourceLocation, Integer>> it = resident.entrySet().iterator();
                    it.hasNext() && residentBytes[index] > BUDGET_BYTES[index];)
                {
                    Map.Entry<ResourceLocation, Integer> eldest = it.next();
                    Long drawn;
                    synchronized(TOUCHED_AT[index])
                    {
    ...
    public static void clear()
    {
        ...
        for(int index = 0; index < CardImageManager.CLASSES; index++)
        {
            ...
            synchronized(resident)
            {
                for(ResourceLocation id : resident.keySet())
                {
                    if(client != null && client.getTextureManager() != null)
                    {
                        client.getTextureManager().release(id);
