...
public final class CardChooser
{
        ...
        List<Integer> options, int mouseX, int mouseY)
    {
        ...
        List<ResourceLocation> faces = new java.util.ArrayList<>(options.size());
    ...
    public static void drawGrid(GuiGraphicsExtractor extractor, Font font, String title,
        List<ResourceLocation> faces, List<String> names, List<Integer> codes, int mouseX, int mouseY,
        boolean pick)
    {
        ...
        for(int cell = 0; cell < faces.size(); cell++)
        {
            ...
            ResourceLocation face = faces.get(cell);
