...
public class DeckEditorScreen extends Screen
{
    ...
    private void rebuildControls()
    {
            ...
            BUTTON_ROW_H, BANLIST_LABEL, pressed ->
        {
            ...
            if(minecraft != null)
            {
                minecraft.setScreen(new BanlistPickerScreen(this));
            }
        });
        // Named at build time, which is enough: returning from the picker goes
        // through setScreen, and setScreen re-inits this screen, so the row --
        // and this line with it -- is rebuilt on the way back. A choice made in
        // the picker is therefore already reflected the first time the tooltip
        // can be shown again.
    ...
    public void render(net.minecraft.client.gui.GuiGraphics vanillaGraphics, int mouseX, int mouseY, float partialTick)
    {
        ...
        if(filtersOpen)
        {
            ...
            Layering.foreground(poseStack, () -> renderOpenList(poseStack, mouseX, mouseY));
        }
        if(rename != null)
        {
        ...
        int mouseY, int art)
    {
        ...
        net.minecraft.resources.ResourceLocation face = showArt
