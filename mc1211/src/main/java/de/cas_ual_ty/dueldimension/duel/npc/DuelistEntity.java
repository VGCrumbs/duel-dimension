...
public class DuelistEntity extends PathfinderMob
{
    ...
    {
        return damageAllowed(source) && super.hurt(source, amount);
    }
     ...
     * <p>
     * A hook rather than a second {@code hurt} override in the subclass: Java
     * has no way to skip a superclass implementation, so the only way for a
     * subclass to be MORE damageable than its parent is for the parent to ask.
     */
