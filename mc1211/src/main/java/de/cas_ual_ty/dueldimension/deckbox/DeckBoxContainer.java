...
// Port: the sleeves slot was a SlotItemHandler (Forge); YDMItemHandler is a
// vanilla Container so it is a plain anonymous Slot now.
public class DeckBoxContainer extends AbstractContainerMenu
{
    ...
    public DeckBoxContainer(MenuType<?> type, int id, Inventory playerInventory, ItemStack itemStack)
    {
        ...
        addSlot(cardSleevesSlot = new Slot(itemHandler, DeckHolder.SLEEVES_INDEX, 8 + 12 * 18, 168 + 0 * 18)
        {
            @Override
    ...
    public ItemStack quickMoveStack(Player playerIn, int index)
    {
        ...
        else if(original.getItem() == DdItems.CARD)
        {
            //inventory to deck box

            Properties card = DdItems.CARD.getCardHolder(original).getCard();
