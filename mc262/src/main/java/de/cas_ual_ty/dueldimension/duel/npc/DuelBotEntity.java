...
public class DuelBotEntity extends DuelistEntity
{
    ...
    public void die(net.minecraft.world.damagesource.DamageSource source)
    {
        if(!level().isClientSide())
        {
            ...
            // 26.2 wants the level handed in rather than reached for.
     ...
     * {@code CompoundTag}, and the getters return Optionals -- so the presence
     * check the 1.21.1 tree writes out is ifPresent here.
     */
    @Override
    ...
    protected void readAdditionalSaveData(ValueInput input)
    {
        ...
        input.getString("Program").ifPresent(value -> entityData.set(PROGRAM, value));
        input.getString("Deck").ifPresent(value -> entityData.set(DECK, value));
        // Absent stays ON -- a bot placed before this existed was never asked,
        // and the answer for a bot nobody asked is the default.
        entityData.set(DESTINY, input.getBooleanOr("DestinyDraw", true));
    }
}
