...
public class ActionIcon
{
    ...
    public ActionIcon(Identifier sourceFile, int fileSize, int iconX, int iconY, int iconWidth, int iconHeight)
    {
    ...
    public String getLocalKey()
    {
        if(localKey == null)
        {
            Identifier rl = DdDuelRegistries.ACTION_ICONS.getKey(this);
    ...
    public ActionIcon(Identifier sourceFile, int size)
    {
        this(sourceFile, size, 0, 0, size, size);
    }
    
    public ActionIcon(Identifier sourceFile, int fileSize, int iconWidth, int iconHeight, byte iconIndex)
    {
