...
/**
 * A creative-only supply block: a full player inventory and no storage of its
 * own, plus a card-picker screen that asks the server (via
 * {@link CardSupplyMessages.RequestCard}) to mint any card into the player's
 * hands. The one datum its constructor needs — which block — arrives as
 * {@link de.cas_ual_ty.dueldimension.net.MenuData} ahead of the menu, read here
 * exactly as the Forge {@code IContainerFactory} buffer was.
 */
public class CardSupplyContainer extends AbstractContainerMenu
{
    ...
    {
        player.addItem(DdItems.CARD.createItemForCardHolder(new CardHolder(card, imageIndex, Rarities.SUPPLY.name)));
    }
