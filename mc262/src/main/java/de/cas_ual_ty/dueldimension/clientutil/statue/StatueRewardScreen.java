...
public final class StatueRewardScreen extends Screen
{
    ...
    private static final Identifier ARROW = gui("arrow.png");
    /**
     ...
     */
    private static final Identifier BACKGROUND = gui("background.png");
    /** The middle of menu_01_00, the disc's own gold plaque. See drawDone. */
    private static final Identifier DONE = gui("button.png");
    /** comm_09_00, the ornate band across the bottom. Sits flush: 339 + 141 = 480. */
    private static final Identifier TRIM = gui("trim.png");
    /** menu_02_01/02/03 -- the game's own three-piece message window. */
    private static final Identifier DIALOG_TOP = gui("dialog_top.png");
    private static final Identifier DIALOG_BODY = gui("dialog_body.png");
    private static final Identifier DIALOG_BOTTOM = gui("dialog_bottom.png");
     ...
     * 162 px.
     */
    private static final Identifier DOD_FONT = Identifier.fromNamespaceAndPath(
        ...
        float partialTick)
    {
        ...
        DdBlitUtil.blit(graphics, BACKGROUND, 0, 0, width, height,
            0F, 0F, 1F, 1F, DdBlitUtil.NO_TINT);

        // NO SHIMMER LAYER. It was a guess and it was covering the thing it
        // sat on.
        //
        ...
        //
        // It was never in the game either. From the disc's own reward-screen
        // constructor, read out of the XBE: "Neither is animated by the ctor --
        // both are placed once and never touched, so the 'scrolling gold'
        // impression is not something this function does." Whether anything
        // ELSE animates them is listed as an open question, and an open question
        // is not a licence to invent an answer.
        ...
        if(!granted.isEmpty())
        {
            ...
            de.cas_ual_ty.dueldimension.clientutil.Layering.foreground(graphics, () -> drawHeldCardInfo(graphics, held));
            de.cas_ual_ty.dueldimension.clientutil.Layering.foreground(graphics, () -> drawMenu(graphics, mouseX, mouseY));
    ...
    private void drawDone(GuiGraphicsExtractor graphics, int mouseX, int mouseY)
    {
        ...
        Component label = Component.literal("Done").setStyle(
            net.minecraft.network.chat.Style.EMPTY.withFont(
                new net.minecraft.network.chat.FontDescription.Resource(DOD_FONT)));
    ...
    private void drawMessageWindow(GuiGraphicsExtractor graphics)
    {
        ...
        // a coordinate by construction and no scale can open a seam.
        int yTop = cy(top);
        ...
        Component prompt = Component.literal(line).setStyle(
            net.minecraft.network.chat.Style.EMPTY.withFont(
                // 26.2 takes a FontDescription where 1.21.1 takes the id
                // directly. The wrapper is the only difference.
                new net.minecraft.network.chat.FontDescription.Resource(DOD_FONT)));
        ...
        boolean doubleClick)
    {
        ...
        if(event.button() == 0 && hit(doneRect(), event.x(), event.y()))
        {
            onClose();
            return true;
        }
        if(event.button() == 1 && !granted.isEmpty())
        {
            return openMenu(event.x(), event.y());
        }
        if(event.button() == 0 && clickArrows(event.x(), event.y()))
        {
            return true;
        }
        // Anywhere between the pointers is the statue at the front, which is
        // the thing being offered -- so clicking it takes it.
        if(event.button() == 0 && betweenPointers(event.x(), event.y()))
        {
            choose();
            return true;
        }
    ...
    public boolean keyPressed(net.minecraft.client.input.KeyEvent event)
    {
        ...
        if(event.key() == org.lwjgl.glfw.GLFW.GLFW_KEY_KP_DECIMAL
        ...
        if(lightEditor != null && lightEditor.keyPressed(event.key()))
        {
            return true;
        }
        if(pressArrows(event.key()))
        {
            return true;
        }
