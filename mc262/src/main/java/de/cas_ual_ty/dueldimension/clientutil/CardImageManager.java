 ...
 * reached by {@link Identifier}, not by a handle, so this class works by
 * pre-registering under the Identifier and handing the Identifier back. An
 * Identifier that reaches a blit while {@code TextureManager} does not hold it
...
public final class CardImageManager
{
     ...
     * part of the Identifier's path ({@code textures/item/{size}/…}), so a
     ...
     */
    record LoadParameter(Identifier id, int sizeIndex, int size, long timestamp,
        boolean requeued)
    {
    }

    /** One finished decode on its way to the GPU. {@code image_manager.h}:74-79. */
    record LoadReturn(loadStatus status, Identifier id, int sizeIndex, int size,
    ...
    static final Map<Identifier, preloadStatus>[] tMap = new Map[CLASSES];
    ...
    /**
     * The Identifier to draw this card with, which is the placeholder until the
     ...
     *                   queued
     */
    public static Identifier peekTextureCard(Identifier id, int size, boolean mayRequest)
    {
        if(id == null)
        {
        ...
        if(!mayRequest)
        {
            Map<Identifier, preloadStatus> map = tMap[classOf(size)];
            synchronized(map)
            {
    ...
    public static Identifier getTextureCard(Identifier id, int size)
    {
        if(id == null)
        {
            return DuelTextures.UNKNOWN;
        ...
        Map<Identifier, preloadStatus> map = tMap[index];
        synchronized(map)
        {
    ...
    private static void loadTexture(Minecraft client, int index)
    {
        ...
        for(int i = 0; i < MAX_IMAGES_PER_FRAME[index]; i++)
        {
            ...
            // callers an Identifier with no texture behind it, which is the
            ...
            if(width != loaded.size() || height != loaded.size())
            {
                ...
                // Identifier's path, so a re-decode yields the same mismatch --
            ...
            // the Identifier resolvable. This is the figure that decides
            ...
            // admit time, so a warmed-but-never-drawn Identifier counted against
     ...
     * {@link #getTextureCard} would keep handing the Identifier out, the blit
     ...
     */
    static void forget(Identifier id, int sizeIndex)
    {
        Map<Identifier, preloadStatus> map = tMap[sizeIndex];
        synchronized(map)
        {
            map.remove(id);
        }
    ...
    {
        for(Map<Identifier, preloadStatus> map : tMap)
        {
            synchronized(map)
            {
                map.clear();
    ...
    private static void mark(int index, Identifier id, preloadStatus status)
    {
        Map<Identifier, preloadStatus> map = tMap[index];
        synchronized(map)
        {
