...
public class BoardPointerScreen extends Screen
{
    ...
    public void tick()
    {
        ...
        // THE HOLD MEANS "DO THE OTHER THING", in both modes.
        //
        // Automatic asks about every chain and holding right-click passes it.
        // Manual passes every chain and holding right-click asks about it. One
        // rule either way round, and no second key to learn.
        //
        // Only ever an OPTIONAL window. A forced chain is one the engine will
        // not take no for, so skipping it would leave the duel sitting on a
        // question that had already been refused -- cancelable() is exactly
        // that flag, straight off MSG_SELECT_CHAIN's `forced`.
    ...
    private void drawIconBar(GuiGraphicsExtractor extractor, int mouseX, int mouseY)
    {
        ...
        {
            net.minecraft.resources.Identifier icon = iconFor(choices.get(slot));
            if(icon == null)
            {
                continue;
            }
