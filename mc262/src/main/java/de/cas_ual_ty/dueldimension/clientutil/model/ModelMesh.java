...
public final class ModelMesh
{
    ...
    private static final Map<Identifier, RenderType> BLENDED = new HashMap<>();
    ...
    public record Part(float[] positions, float[] normals, float[] uvs,
        int[] joints, float[] weights, Identifier texture)
    {
     ...
     */
    public static RenderType typeFor(Identifier texture)
    {
     ...
     */
    public static RenderType typeFor(Identifier texture, boolean blend)
    {
        Map<Identifier, RenderType> types = blend ? BLENDED : TYPES;
    ...
    public static ModelMesh bake(GlbModel model, String name)
    {
        ...
        for(GlbModel.Primitive primitive : model.primitives())
        {
            ...
            // an Identifier below, once the images have been registered.
        ...
        List<Identifier> textures = register(model, name);
        for(int i = 0; i < parts.size(); i++)
        {
            int image = partImages.get(i);
            Identifier texture = image >= 0 && image < textures.size()
     ...
     */
    private static List<Identifier> register(GlbModel model, String name)
    {
