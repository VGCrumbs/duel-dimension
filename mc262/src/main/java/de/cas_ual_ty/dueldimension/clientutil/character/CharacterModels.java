...
public final class CharacterModels
{
    ...
    {
        Identifier id = Identifier.fromNamespaceAndPath(DuelDimension.MOD_ID, path);
     ...
     */
    public static Identifier texture(CharacterLook look, String slot)
    {
        ...
        Identifier shipped = baked.parts().get(part).texture();
        if("disc".equals(slot))
        {
            return shipped;
        }
        ...
        for(int y = 0; y < source.height(); y++)
        {
            ...
            {
                // 26.2's setPixel takes ARGB, which is what the palette holds.
                // 1.21.1's setPixelRGBA takes ABGR; see PORTING.md.
                image.setPixel(x, y, pixels[y * source.width() + x]);
            }
        }
    ...
    public static void forget(CharacterLook look)
    {
        for(String slot : SLOTS)
        {
            Identifier id = TEXTURES.remove(look.key() + "/" + slot);
            if(id != null)
            {
