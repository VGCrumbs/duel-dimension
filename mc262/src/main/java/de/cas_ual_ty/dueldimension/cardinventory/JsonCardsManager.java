...
public abstract class JsonCardsManager
{
    ...
    {
        if(DuelDimension.commonConfig().logBinderIO.get())
        {
            DuelDimension.log("Trying to load card inventory from file: " + getFile().getAbsolutePath());
        }
        ...
        if(isSafed())
        {
            if(DuelDimension.commonConfig().logBinderIO.get())
            {
                DuelDimension.log("File will load!");
            }
            ...
            Task t = new Task(TaskPriority.BINDER_LOAD, () ->
            {
                ...
                if(DuelDimension.commonConfig().logBinderIO.get())
                {
                    DuelDimension.log("Done loading from file: " + getFile().getAbsolutePath());
                }
                setIdle();
                callback.run();
            });
    ...
    public void safe(Runnable callback)
    {
        if(DuelDimension.commonConfig().logBinderIO.get())
        {
            DuelDimension.log("Trying to save card inventory to file: " + getFile().getAbsolutePath());
        }
        ...
        if(isLoaded())
        {
            if(DuelDimension.commonConfig().logBinderIO.get())
            {
                DuelDimension.log("File will safe!");
            }
            ...
            Task t = new Task(TaskPriority.BINDER_SAVE, () ->
            {
                ...
                if(DuelDimension.commonConfig().logBinderIO.get())
                {
                    DuelDimension.log("Done saving to file: " + getFile().getAbsolutePath());
                }
                setIdle();
                callback.run();
            });
    ...
    protected JsonArray loadFromFile()
    {
        ...
        else
        {
            try
            {
                return DdIOUtil.parseJsonFile(file).getAsJsonArray();
            }
