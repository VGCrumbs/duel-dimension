...
public class DeckBoxItem extends Item implements MenuProvider
{
    ...
    {
        return itemStack.getCapability(DuelDimension.CARD_ITEM_INVENTORY).orElse(null);
    }
    ...
    {
        return new DeckBoxContainer(DdContainerTypes.DECK_BOX.get(), id, playerInv, DeckBoxItem.getActiveDeckBox(player));
    }
    ...
    public void setDeckHolder(ItemStack itemStack, DeckHolder holder)
    {
        ...
        for(int i = 0; i < holder.getMainDeckSize(); ++i)
        {
            ...
            if(c != null)
            {
                itemHandler.setStackInSlot(DeckHolder.MAIN_DECK_INDEX_START + i, DdItems.CARD.get().createItemForCardHolder(c));
            }
        }
        ...
        for(int i = 0; i < holder.getExtraDeckSize(); ++i)
        {
            ...
            if(c != null)
            {
                itemHandler.setStackInSlot(DeckHolder.EXTRA_DECK_INDEX_START + i, DdItems.CARD.get().createItemForCardHolder(c));
            }
        }
        ...
        for(int i = 0; i < holder.getSideDeckSize(); ++i)
        {
            ...
            if(c != null)
            {
                itemHandler.setStackInSlot(DeckHolder.SIDE_DECK_INDEX_START + i, DdItems.CARD.get().createItemForCardHolder(c));
            }
        }
    ...
    public void readShareTag(ItemStack stack, @Nullable CompoundTag nbt)
    {
        ...
        {
            stack.getCapability(DuelDimension.CARD_ITEM_INVENTORY).ifPresent(handler ->
            {
