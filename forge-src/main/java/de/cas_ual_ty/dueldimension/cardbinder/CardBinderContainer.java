...
public class CardBinderContainer extends AbstractContainerMenu
{
    ...
    {
        this(type, id, playerInventory, null, DdItems.CARD_BINDER.get().getActiveBinder(playerInventory.player));
    ...
    public CardBinderContainer(MenuType<?> type, int id, Inventory playerInventory, UUIDCardsManager manager, ItemStack itemStack)
    {
        ...
        addSlot(insertionSlot = new Slot(containerInv, 0, 179, 18)
        {
            ...
            {
                return stack.getItem() == DdItems.CARD.get() && DdItems.CARD.get().getCardHolder(stack).getCard() != null;
            }
            ...
            public void set(ItemStack stack)
            {
                if(serverList != null)
                {
                    ...
                    serverList.addCard(DdItems.CARD.get().getCardHolder(stack), page);
    ...
    {
        DuelDimension.channel.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) player), new CardBinderMessages.UpdateList(page, serverList.getCardsForPage(page)));
    }
    
    protected void updatePagesToClient()
    {
        DuelDimension.channel.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) player), new CardBinderMessages.UpdatePage(page, serverList.getPagesAmount()));
    }
    ...
    public void indexClicked(int index)
    {
        ...
        if(card != null)
        {
            ItemStack itemStack = DdItems.CARD.get().createItemForCardHolder(card);
