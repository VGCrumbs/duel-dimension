...
public class DdCardResourcePack extends FilePackResources
{
    public static final String PATH_PREFIX = "assets/" + DuelDimension.MOD_ID + "/textures/item/";
    ...
    {
        if(DdCardResourcePack.OS_WINDOWS)
        {
            s = DdCardResourcePack.BACKSLASH_MATCHER.replaceFrom(s, '/');
        }
    ...
    protected InputStream getResource(String resourcePath) throws IOException
    {
        ...
        File image = getFile(DdCardResourcePack.convertPath(resourcePath));
    ...
    private File getFile(String filename)
    {
        ...
        if(!filename.startsWith(DdCardResourcePack.PATH_PREFIX))
        {
            return null;
        }
        
        // We remove that prefix part
        filename = filename.substring(DdCardResourcePack.PATH_PREFIX.length());
    ...
    {
        return type == PackType.CLIENT_RESOURCES ? ImmutableSet.of(DuelDimension.MOD_ID) : ImmutableSet.of();
    }
    ...
    public Collection<ResourceLocation> getResources(PackType type, String namespaceIn, String pathIn, Predicate<ResourceLocation> filterIn)
    {
        ...
        if(type == ResourcePackType.CLIENT_RESOURCES)
        {
            ...
            if(listFiles != null)
            {
                ...
                {
                    list.add(new ResourceLocation(DuelDimension.MOD_ID, f.getName().replace(this.filter.getRequiredSuffix(), "")));
                }
            }
        }
        */
