---
aliases: [Crumby Dueling, Project Hub]
tags: [project, minecraft, yugioh, ocgcore]
created: 2026-08-04
status: active
---

# Crumby Dueling — Project Hub
...
- [x] JNA binding for ocgcore API v11 (`de.cas_ual_ty.ydm.ocg`)
...
- [ ] Query API binding → `BoardState` snapshots
- [ ] **`FilteredBoardState` first** — legal-player knowledge only; omniscience gated behind a profile knob
- [ ] `HeuristicBot`: trade evaluation, lethal detection, backrow discipline, resource conservation
- [ ] `gradlew botArena` — seeded winrate tables (the permanent regression metric)
- **Gate:** HeuristicBot ≥ 90% vs RandomBot over 500 duels
...
EDOPro's rules engine loaded **in-process** via JNA. The binding (`de.cas_ual_ty.ydm.ocg`) is deliberately Minecraft-free so everything below Phase 5 runs headless.
...
| Thing | Where |
|---|---|
| Repo | `C:\Users\Admin\Desktop\YGO\CrumbyDueling` (branch `crumby`, remote `upstream` = CAS-ual-TY) |
