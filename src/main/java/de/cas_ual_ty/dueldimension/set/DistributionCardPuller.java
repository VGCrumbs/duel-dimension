...
import de.cas_ual_ty.dueldimension.set.Distribution.Pull;
import de.cas_ual_ty.dueldimension.set.Distribution.Pull.PullEntry;
...
public class DistributionCardPuller extends CardPuller
{
    ...
    public DistributionCardPuller(JsonObject setJson, CardSet set) throws IllegalArgumentException
    {
        ...
        distribution = DdDatabase.DISTRIBUTIONS_LIST.get(distributionName);
    ...
    public List<ItemStack> open(Random random)
    {
        ...
        {
            list.add(DdItems.CARD.get().createItemForCardHolder(c));
        }
    ...
    public void logErrors()
    {
        for(Pull pull : distribution.pulls)
        {
            ...
            for(PullEntry pe : pull.pullEntries)
            {
                ...
                {
                    DuelDimension.log("Set " + set.code + ": One pull entry has no rarities or count = 0 and will not do anything.");
                    continue;
                }
                ...
                if(cardPool.size() < pe.count)
                {
                    DuelDimension.log("Set " + set.code + ": Not enough cards for rarities: " + s.toString());
                    continue;
                }
                ...
                if(uniqueCards < pe.count)
                {
                    DuelDimension.log("Set " + set.code + ": Not enough unique cards for rarities (will contain duplicates): " + s.toString());
                }
            }
        }
        ...
        for(String rarity : set.rarityPool)
        {
            ...
            {
                DuelDimension.log("Set " + set.code + ": Rarity and the cards using it can never be pulled: " + rarity);
            }
        }
    }
