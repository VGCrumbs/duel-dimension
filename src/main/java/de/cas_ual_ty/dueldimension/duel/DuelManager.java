...
public class DuelManager
{
    ...
    public DuelManager(boolean isRemote, Supplier<DuelMessageHeader> header)
    {
        ...
        random = new Random(DuelDimension.random.nextLong());
        reset();
    }
    ...
    {
        // closeContainer is protected on Player and public on ServerPlayer.
        // Kicking someone out of a duel only ever happens server-side, so the
        // cast is the honest form of what this always did.
    ...
    public List<DeckSource> getAvailableDecksFor(Player player)
    {
        ...
        FindDecksEvent.FIND.invoker().findDecks(e);
    ...
    {
        sendInfoMessageToAll(Component.translatable("container.dueldimension.duel.info_start"));
        populatePlayField();
    ...
    public void playerOffersDraw(Player player)
    {
        ...
        {
            DdUtil.executeDrawCommands(player1, player2);
    ...
    public void playerAdmitsDefeat(Player player)
    {
        ...
        {
            DdUtil.executeAdmitDefeatCommands(player2, player1);
            end = true;
        }
        else if(role == PlayerRole.PLAYER2 && setPlayerAdmitsDefeat(player, role, true))
        {
            DdUtil.executeAdmitDefeatCommands(player1, player2);
            end = true;
        }
    ...
    {
        return Component.translatable("container.dueldimension.duel.info_name")
    ...
    /**
     * Sends one duel message to one player.
     * <p>
     * Was generic over {@code MSG} because Forge's channel took anything it had
     * been registered with. A payload is a type here, so the envelope is put on
     * in one place -- {@link de.cas_ual_ty.dueldimension.duel.network.DuelPayloads}
     * -- and this takes the thing it actually sends.
     */
    protected void sendGeneralPacketTo(ServerPlayer player, DuelMessage msg)
    {
        de.cas_ual_ty.dueldimension.duel.network.DuelPayloads.send(player, msg);
    }
