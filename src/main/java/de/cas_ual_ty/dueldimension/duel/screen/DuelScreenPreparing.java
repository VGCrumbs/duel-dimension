...
public class DuelScreenPreparing<E extends DuelContainer> extends DuelContainerScreen<E>
{
    ...
    protected void init()
    {
        ...
        if(!deckChosen && getZoneOwner() != ZoneOwner.NONE)
        {
            ...
            addRenderableWidget(chooseDeckButton = new ImprovedButton(x - chooseWidth / 2, topPos + imageHeight - 20 - 10, chooseWidth, 20, Component.translatable("container." + DuelDimension.MOD_ID + ".duel.choose_deck"), (button) -> chooseDeckClicked(), this::chooseDeckTooltip));
    ...
    protected void drawActiveDeckForeground(PoseStack ms, int mouseX, int mouseY)
    {
        ...
        if(h != DeckWrapper.DUMMY)
        {
            ...
            if(d != null && d != DeckHolder.DUMMY)
            {
                ...
                // main deck
                //drawString
                font.draw(ms, Component.translatable("container.dueldimension.deck_box.main").append(" " + d.getMainDeckSize() + "/" + DeckHolder.MAIN_DECK_SIZE), guiLeft + 8F, guiTop + 6F, 0x404040);
                
                // extra deck
                //drawString
                font.draw(ms, Component.translatable("container.dueldimension.deck_box.extra").append(" " + d.getExtraDeckSize() + "/" + DeckHolder.EXTRA_DECK_SIZE), guiLeft + 8F, guiTop + 92F, 0x404040);
                
                // side deck
                //drawString
                font.draw(ms, Component.translatable("container.dueldimension.deck_box.side").append(" " + d.getSideDeckSize() + "/" + DeckHolder.SIDE_DECK_SIZE), guiLeft + 8F, guiTop + 124F, 0x404040);
                ...
                for(int y = 0; y < DeckHolder.MAIN_DECK_SIZE / itemsPerRow; ++y)
                {
                    for(int x = 0; x < itemsPerRow && x + y * itemsPerRow < DeckHolder.MAIN_DECK_SIZE; ++x)
                    {
                        ...
                        if(c != null && c.getCard() != null)
                        {
                            ...
                            DdBlitUtil.fullBlit(ms, guiLeft + offX, guiTop + offY, 16, 16);
    ...
    protected void drawActiveDeckBackground(PoseStack ms, float partialTicks, int mouseX, int mouseY)
    {
        ...
        if(h != DeckWrapper.DUMMY)
        {
            ...
            if(d != null && d != DeckHolder.DUMMY)
            {
                ...
                DdBlitUtil.blit(ms, guiLeft, guiTop, xSize, ySize, 0, 0, xSize, ySize, 512, 256);
                DdBlitUtil.blit(ms, guiLeft, guiTop + ySize, xSize, 7, 0, 243, xSize, 7, 512, 256);
            }
        }
    }
    ...
    {
        renderTooltip(ms, Component.translatable("container.dueldimension.duel.choose_deck"), mouseX, mouseY);
    }
    ...
    {
        DuelDimension.channel.send(PacketDistributor.SERVER.noArg(), new DuelMessages.ChooseDeck(getHeader(), getActiveDeckWrapper().index));
    }
    
    public void requestDeck(int index)
    {
        DuelDimension.channel.send(PacketDistributor.SERVER.noArg(), new DuelMessages.RequestDeck(getHeader(), index));
    }
    
    protected static class DeckWrapper
    {
        public static final DeckWrapper DUMMY = new DeckWrapper(new DeckSource(DeckHolder.DUMMY, new ItemStack(DdItems.BLANC_CARD.get())), -1);
