...
public final class WornOutfits
{
    ...
    public static void broadcast(ServerPlayer player)
    {
        ...
        OutfitMessages.Worn worn = new OutfitMessages.Worn(player.getUUID(),
            de.cas_ual_ty.dueldimension.duel.profile.DuelProfiles.get(player).outfit());
        // Fabric has no PacketDistributor.ALL: sending to everyone is sending
        // to each of them, which is what ALL did anyway.
        for(ServerPlayer everyone : server.getPlayerList().getPlayers())
        {
            net.fabricmc.fabric.api.networking.v1.ServerPlayNetworking.send(everyone, worn);
        }
    }
}
