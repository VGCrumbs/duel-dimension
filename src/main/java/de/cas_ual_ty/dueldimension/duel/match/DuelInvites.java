...
public final class DuelInvites
{
    ...
    public static String invite(ServerPlayer from, ServerPlayer to)
    {
        ...
        PENDING.put(to.getUUID(), new Invite(from.getUUID(), from.getGameProfile().getName(),
            machine, to.server.overworld().getGameTime() + EXPIRY_TICKS));
    ...
    public static String accept(ServerPlayer target, String challengerName)
    {
        ...
        invite.machine.moveTo(MatchState.DUELING);

        String error = DuelistDuels.startPlayerDuel(challenger, target);
        if(error != null)
        {
