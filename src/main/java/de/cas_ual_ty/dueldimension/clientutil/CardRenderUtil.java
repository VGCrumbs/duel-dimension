...
import de.cas_ual_ty.dueldimension.rarity.RarityLayerType;
...
public class CardRenderUtil
{
    ...
    {
        return new ResourceLocation(DuelDimension.MOD_ID, "textures/item/" + ClientProxy.activeCardInfoImageSize + "/" + DdItems.CARD_BACK.getId().getPath() + ".png");
    }
    
    public static ResourceLocation getMainCardBack()
    {
        return new ResourceLocation(DuelDimension.MOD_ID, "textures/item/" + ClientProxy.activeCardMainImageSize + "/" + DdItems.CARD_BACK.getId().getPath() + ".png");
    }
    
    public static ResourceLocation getInfoTokenOverlay()
    {
        return new ResourceLocation(DuelDimension.MOD_ID, "textures/item/" + ClientProxy.activeCardInfoImageSize + "/" + "token_overlay" + ".png");
    }
    
    public static ResourceLocation getMainTokenOverlay()
    {
        return new ResourceLocation(DuelDimension.MOD_ID, "textures/item/" + ClientProxy.activeCardMainImageSize + "/" + "token_overlay" + ".png");
    }
    ...
    public static void renderInfoCardWithRarity(PoseStack ms, int mouseX, int mouseY, float x, float y, float width, float height, CardHolder card)
    {
        ...
        CardRenderUtil.bindInfoResourceLocation(card);
        DdBlitUtil.fullBlit(ms, x - width / 2, y - height / 2, width, height);
        
        RarityEntry rarity = DdDatabase.getRarity(card.getRarity());
        ...
        if(rarity != null)
        {
            for(RarityLayer layer : rarity.layers)
            {
                ...
                Runnable mask = () ->
                {
                    ...
                    DdBlitUtil.fullBlit(ms, mouseX - width / 2, mouseY - height / 2, width, height);
                };
                ...
                DdBlitUtil.advancedMaskedBlit(ms, x, y, width, height, mask, renderer, layer.type.invertedRendering);
            }
        }
    }
    
    public static void renderDuelCardAdvanced(PoseStack ms, CardSleevesType back, int mouseX, int mouseY, float x, float y, float width, float height, DuelCard card, DdBlitUtil.FullBlitMethod blitMethod, boolean forceFaceUp)
    {
    ...
    public static void renderDuelCardAdvanced(PoseStack ms, CardSleevesType back, int mouseX, int mouseY, float x, float y, float width, float height, DuelCard card, CardPosition position, DdBlitUtil.FullBlitMethod blitMethod)
    {
        Minecraft mc = ClientProxy.getMinecraft();
