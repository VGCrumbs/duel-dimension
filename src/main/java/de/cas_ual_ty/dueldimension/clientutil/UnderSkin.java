...
public final class UnderSkin
{
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(
        de.cas_ual_ty.dueldimension.DuelDimension.MOD_ID, "underskin");
    ...
    private static void load(Path path)
    {
        ...
        try(java.io.InputStream stream = Files.newInputStream(path))
        {
            ...
            // A texture carries a label for the debug overlay and crash reports
            // now, and it is a supplier so building the string costs nothing
            // unless something actually asks.
            loaded = new DynamicTexture(() -> TEXTURE.toString(), image);
    ...
    private static NativeImage widen(NativeImage old)
    {
        ...
        for(int y = 0; y < 32; y++)
        {
            for(int x = 0; x < 64; x++)
            {
                wide.setPixel(x, y, old.getPixel(x, y));
            }
        }
