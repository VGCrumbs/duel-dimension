...
import de.cas_ual_ty.dueldimension.*;
...
import de.cas_ual_ty.dueldimension.card.InspectCardScreen;
...
import de.cas_ual_ty.dueldimension.duel.screen.DuelContainerScreen;
import de.cas_ual_ty.dueldimension.duel.screen.DuelScreenBase;
...
public class ClientProxy implements ISidedProxy
{
    ...
    public void preInit()
    {
        ...
        {
            ClientProxy.getMinecraft().getResourcePackRepository().addPackFinder(new DdResourcePackFinder());
        }
    }
    ...
    public void init()
    {
        DuelDimension.log("Sizes from client config (info/item/main): " + ClientProxy.activeCardInfoImageSize + " / " + ClientProxy.activeCardItemImageSize + " (" + ClientProxy.itemsUseCardImages + ") / " + ClientProxy.activeCardMainImageSize);
        ...
        if(ClientProxy.itemsUseCardImages)
        {
            try
            {
                ...
                if(list.size() == 0)
                {
                    DuelDimension.log("Items will use card images!");
                    ClientProxy.itemsUseCardImagesActive = true;
                }
                else
                {
                    DuelDimension.log("Items will not use card images, still missing " + list.size() + " images. Fetching...");
            ...
            {
                DuelDimension.log("Failed checking missing item images!");
                e.printStackTrace();
                ClientProxy.itemsUseCardImagesFailed = true;
            }
        }
        ...
        if(ClientProxy.itemsUseSetImages)
        {
            try
            {
                ...
                if(list.size() == 0)
                {
                    DuelDimension.log("Items will use set images!");
                    ClientProxy.itemsUseSetImagesActive = true;
                }
                else
                {
                    DuelDimension.log("Items will not use set images, still missing " + list.size() + " images. Fetching...");
            ...
            {
                DuelDimension.log("Failed checking missing set images!");
                e.printStackTrace();
                ClientProxy.itemsUseSetImagesFailed = true;
            }
        }
        
        MenuScreens.register(DdContainerTypes.CARD_BINDER.get(), CardBinderScreen::new);
        MenuScreens.register(DdContainerTypes.DECK_BOX.get(), DeckBoxScreen::new);
        MenuScreens.<DuelContainer, DuelContainerScreen<DuelContainer>>register(DdContainerTypes.DUEL_BLOCK_CONTAINER.get(), DuelScreenBase::new);
        MenuScreens.<DuelContainer, DuelContainerScreen<DuelContainer>>register(DdContainerTypes.DUEL_ENTITY_CONTAINER.get(), DuelScreenBase::new);
        MenuScreens.register(DdContainerTypes.CARD_SUPPLY.get(), CardSupplyScreen::new);
        MenuScreens.register(DdContainerTypes.CARD_SET.get(), (MenuScreens.ScreenConstructor<CIIContainer, CIIScreen<CIIContainer>>) (CIIScreen::new));
        MenuScreens.register(DdContainerTypes.CARD_SET_CONTENTS.get(), (MenuScreens.ScreenConstructor<CIIContainer, CIIScreen<CIIContainer>>) (CIIScreen::new));
        MenuScreens.register(DdContainerTypes.SIMPLE_BINDER.get(), (MenuScreens.ScreenConstructor<CIIContainer, CIIScreen<CIIContainer>>) (CIIScreen::new));
    ...
    public void initFolders()
    {
        ...
        ClientProxy.rawRarityImagesFolder = new File(DuelDimension.mainFolder, "rarity_images");
        
        DdIOUtil.createDirIfNonExistant(ClientProxy.imagesParentFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.cardImagesFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.setImagesFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.rarityImagesFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.rawCardImagesFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.rawSetImagesFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.rawRarityImagesFolder);
    }
    ...
    public void initFiles() // done before #init
    {
        ...
        DdIOUtil.createDirIfNonExistant(ClientProxy.cardInfoImagesFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.cardItemImagesFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.cardMainImagesFolder);
        
        DdIOUtil.createDirIfNonExistant(ClientProxy.rarityMainImagesFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.rarityInfoImagesFolder);
        ...
        DdIOUtil.createDirIfNonExistant(ClientProxy.setInfoImagesFolder);
        DdIOUtil.createDirIfNonExistant(ClientProxy.setItemImagesFolder);
    }
    ...
    private void textureStitchPre(TextureStitchEvent.Pre event)
    {
                ...
                || (ClientProxy.itemsUseSetImages && !ClientProxy.itemsUseSetImagesFailed && !ClientProxy.itemsUseSetImagesActive))
        {
            ...
            {
                flag = true;
                DuelDimension.log("Sleeping for a couple seconds to give the worker enough time to check the item and set images...");
            }
            
            // sometimes this gets done before DuelDimension.itemsUseCardImagesActive is set to true
            ...
            {
                DuelDimension.log("Tried sleeping to give textures enough time... It didnt work :(");
                e.printStackTrace();
                break;
            }
        }
        ...
        if(i > 0)
        {
            DuelDimension.log("Slept for " + i + " seconds.");
        }
        
        if(ClientProxy.itemsUseCardImagesActive)
        {
            DuelDimension.log("Stitching " + DdDatabase.getTotalCardsAndVariants() + " card item textures!");
        ...
        {
            DuelDimension.log("Stitching " + DdDatabase.SETS_LIST.size() + " set item textures!");
    ...
    private void modelRegistry(ModelEvent.RegisterAdditional event)
    {
        ...
        while((ClientProxy.activeCardItemImageSize == 0 || ClientProxy.activeSetItemImageSize == 0))
        {
            ...
            {
                flag = true;
                DuelDimension.log("Sleeping for a couple seconds to give the worker enough time to read the config...");
            }
        ...
        DuelDimension.log("Registering models (size: " + ClientProxy.activeCardItemImageSize + ") for " + DdItems.BLANC_CARD.getId().toString() + " and " + DdItems.CARD_BACK.getId().toString());
        
        // 16 is default texture; no need to do anything special in that case
        if(ClientProxy.activeCardItemImageSize != 16)
        {
            
            event.register(new ModelResourceLocation(new ResourceLocation(DdItems.BLANC_CARD.getId().toString() + "_" + ClientProxy.activeCardItemImageSize), "inventory"));
            event.register(new ModelResourceLocation(new ResourceLocation(DdItems.CARD_BACK.getId().toString() + "_" + ClientProxy.activeCardItemImageSize), "inventory"));
        ...
        DuelDimension.log("Registering models (size: " + ClientProxy.activeSetItemImageSize + ") for " + DdItems.BLANC_SET.getId().toString());
        
        if(ClientProxy.activeSetItemImageSize != 16)
        {
            event.register(new ModelResourceLocation(new ResourceLocation(DdItems.BLANC_SET.getId().toString() + "_" + ClientProxy.activeSetItemImageSize), "inventory"));
        }
    }
    
    private void modelBake(ModelEvent.BakingCompleted event)
    {
        DuelDimension.log("Baking models (size: " + ClientProxy.activeCardItemImageSize + ") for " + DdItems.BLANC_CARD.getId().toString() + " and " + DdItems.CARD_BACK.getId().toString());
        
        // 16 is default texture; no need to do anything special in that case
        if(ClientProxy.activeCardItemImageSize != 16)
        {
            event.getModels().put(new ModelResourceLocation(DdItems.BLANC_CARD.getId(), "inventory"),
                    event.getModelManager().getModel(
                            new ModelResourceLocation(
                                    new ResourceLocation(DdItems.BLANC_CARD.getId().toString() + "_" + ClientProxy.activeCardItemImageSize), "inventory")));
            
            event.getModels().put(new ModelResourceLocation(DdItems.CARD_BACK.getId(), "inventory"),
                    event.getModelManager().getModel(
                            new ModelResourceLocation(
                                    new ResourceLocation(DdItems.CARD_BACK.getId().toString() + "_" + ClientProxy.activeCardItemImageSize), "inventory")));
            
            for(CardSleevesType sleeves : CardSleevesType.VALUES)
            {
                if(!sleeves.isCardBack())
                {
                    event.getModels().put(new ModelResourceLocation(new ResourceLocation(DuelDimension.MOD_ID, "sleeves_" + sleeves.name), "inventory"),
                            event.getModelManager().getModel(
                                    new ModelResourceLocation(
        ...
        if(ClientProxy.activeSetItemImageSize != 16)
        {
            event.getModels().put(new ModelResourceLocation(DdItems.BLANC_SET.getId(), "inventory"),
                    event.getModelManager().getModel(
                            new ModelResourceLocation(
                                    new ResourceLocation(DdItems.BLANC_SET.getId().toString() + "_" + ClientProxy.activeSetItemImageSize), "inventory")));
        }
        
        ModelResourceLocation key = new ModelResourceLocation(DdItems.CARD.getId(), "inventory");
        event.getModels().put(key, new CardBakedModel(event.getModelManager().getModel(key)));
        
        key = new ModelResourceLocation(DdItems.SET.getId(), "inventory");
        event.getModels().put(key, new CardSetBakedModel(event.getModelManager().getModel(key)));
        key = new ModelResourceLocation(DdItems.OPENED_SET.getId(), "inventory");
        event.getModels().put(key, new CardSetBakedModel(event.getModelManager().getModel(key)));
    }
    ...
    private void modConfig(ModConfigEvent event)
    {
        ...
        {
            DuelDimension.log("Baking client config!");
            ClientProxy.activeCardInfoImageSize = DdUtil.toPow2ConfigValue(ClientProxy.clientConfig.activeCardInfoImageSize.get(), 4);
            ClientProxy.activeCardItemImageSize = DdUtil.toPow2ConfigValue(ClientProxy.clientConfig.activeCardItemImageSize.get(), 4);
            ClientProxy.activeCardMainImageSize = DdUtil.toPow2ConfigValue(ClientProxy.clientConfig.activeCardMainImageSize.get(), 4);
            ClientProxy.activeSetInfoImageSize = DdUtil.toPow2ConfigValue(ClientProxy.clientConfig.activeSetInfoImageSize.get(), 4);
            ClientProxy.activeSetItemImageSize = DdUtil.toPow2ConfigValue(ClientProxy.clientConfig.activeSetItemImageSize.get(), 4);
    ...
    private void guiScreenDrawScreenPost(ScreenEvent.Render.Post event)
    {
        if(event.getScreen() instanceof AbstractContainerScreen)
        {
            ...
            if(containerScreen.getSlotUnderMouse() != null && !containerScreen.getSlotUnderMouse().getItem().isEmpty())
            {
                ...
                if(itemStack.getItem() == DdItems.CARD.get())
                {
                    CardRenderUtil.renderCardInfo(event.getPoseStack(), DdItems.CARD.get().getCardHolder(itemStack), containerScreen);
                }
                else if(itemStack.getItem() == DdItems.SET.get())
                {
                    renderSetInfo(event.getPoseStack(), DdItems.SET.get().getCardSet(itemStack), containerScreen.getGuiLeft());
                }
                else if(itemStack.getItem() == DdItems.OPENED_SET.get())
                {
                    renderSetInfo(event.getPoseStack(), DdItems.OPENED_SET.get().getCardSet(itemStack), containerScreen.getGuiLeft());
                }
    ...
    private void renderGameOverlayPost(RenderGuiOverlayEvent.Post event)
    {
        ...
        if(getClientPlayer() != null && ClientProxy.getMinecraft().screen == null)
        {
            ...
            if(player.getMainHandItem().getItem() == DdItems.CARD.get())
            {
                CardRenderUtil.renderCardInfo(event.getPoseStack(), DdItems.CARD.get().getCardHolder(player.getMainHandItem()));
            }
            else if(player.getMainHandItem().getItem() == DdItems.SET.get())
            {
                renderSetInfo(event.getPoseStack(), DdItems.SET.get().getCardSet(player.getMainHandItem()));
            }
            else if(player.getMainHandItem().getItem() == DdItems.OPENED_SET.get())
            {
                renderSetInfo(event.getPoseStack(), DdItems.OPENED_SET.get().getCardSet(player.getMainHandItem()));
            }
            ...
            else if(player.getOffhandItem().getItem() == DdItems.CARD.get())
            {
                CardRenderUtil.renderCardInfo(event.getPoseStack(), DdItems.CARD.get().getCardHolder(player.getOffhandItem()));
            }
            else if(player.getOffhandItem().getItem() == DdItems.SET.get())
            {
                renderSetInfo(event.getPoseStack(), DdItems.SET.get().getCardSet(player.getOffhandItem()));
            }
            else if(player.getOffhandItem().getItem() == DdItems.OPENED_SET.get())
            {
                renderSetInfo(event.getPoseStack(), DdItems.OPENED_SET.get().getCardSet(player.getOffhandItem()));
            }
    ...
    private void renderSetInfo(PoseStack ms, CardSet set, int width)
    {
        ...
        DdBlitUtil.fullBlit(ms, x, margin, imageSize, imageSize);
