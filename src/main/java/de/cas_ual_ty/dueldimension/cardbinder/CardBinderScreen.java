...
{
    private static final ResourceLocation CARD_BINDER_GUI_TEXTURE = new ResourceLocation(DuelDimension.MOD_ID, "textures/gui/card_binder.png");
    ...
    protected void init()
    {
        ...
        addRenderableWidget(prevButton = new ImprovedButton(leftPos + imageWidth - 24 - 8 - 27, topPos + 4, 12, 12, Component.translatable("generic.dueldimension.left_arrow"), this::onButtonClicked));
        addRenderableWidget(nextButton = new ImprovedButton(leftPos + imageWidth - 12 - 8 - 27, topPos + 4, 12, 12, Component.translatable("generic.dueldimension.right_arrow"), this::onButtonClicked));
    ...
    protected void onButtonClicked(Button button)
    {
        ...
        {
            DuelDimension.channel.send(PacketDistributor.SERVER.noArg(), new CardBinderMessages.ChangePage(false));
        }
        else if(button == nextButton)
        {
            DuelDimension.channel.send(PacketDistributor.SERVER.noArg(), new CardBinderMessages.ChangePage(true));
        }
        else if(button == reloadButton)
        {
            DuelDimension.channel.send(PacketDistributor.SERVER.noArg(), new CardBinderMessages.ChangeSearch(cardSearch.getValue()));
        }
    }
    ...
    protected void onCardClicked(CardButton button, int index)
    {
        ...
        if(button.getCard() != null)
        {
            DuelDimension.channel.send(PacketDistributor.SERVER.noArg(), new CardBinderMessages.IndexClicked(index));
        }
    }
        ...
        else if(getMenu().loaded)
        {
            if(keyCode == CardBinderScreen.Q)
            {
                for(CardButton button : cardButtons)
                {
                    ...
                    {
                        DuelDimension.channel.send(PacketDistributor.SERVER.noArg(), new CardBinderMessages.IndexDropped(button.index));
                        break;
                    }
                }
            }
